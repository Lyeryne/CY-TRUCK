...
distance_trajet=$(LC_NUMERIC=en_US.UTF-8  awk -F';' 'NR>1 {distance[$1] += $5} END {for (conducteur in distance) printf "%s;%.6f\n", conducteur, distance[conducteur]}' "data/$1" | sort -t';' -k2 -r -n | head -10 | sort -t';' -k1 -n -r) 
...
t=$( { time -p LC_NUMERIC=en_US.UTF-8  awk -F';' 'NR>1 {distance[$1] += $5} END {for (conducteur in distance) printf "%s;%.6f\n", conducteur, distance[conducteur]}' "data/$1" | sort -t';' -k2 -r -n | head -10 | sort -t';' -k1 -n -r; } 2>&1 | awk '/^real/ {print $2}' )
